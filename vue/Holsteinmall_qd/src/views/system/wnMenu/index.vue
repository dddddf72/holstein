...
<script>
  import crudMenu from '@/api/system/wnMenu'
  ...
  // crud交由presenter持有
const defaultCrud = CRUD({ title: '菜单', url: 'api/wnMenus', crudMethod: { ...crudMenu }})
...
export default {
  ...
  data() {
    return {
      menus: [],
      permission: {
        add: ['admin', 'wnMenu:add'],
        edit: ['admin', 'wnMenu:edit'],
        del: ['admin', 'wnMenu:del']
      },
      rules: {
        name: [
