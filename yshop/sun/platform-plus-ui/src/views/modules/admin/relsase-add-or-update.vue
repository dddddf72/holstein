  ...
  <div
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="发布类型 " prop="releaseType">
        <el-radio-group v-model="dataForm.releaseType">
      ...
      <el-form-item v-show="dataForm.releaseType===2" label="视频上传" prop="video">
        <!-- action必选参数, 上传的地址 -->
        <el-upload
          drag
          :action='uploadPath'
          ...
          <video v-if="dataForm.video !='' && videoFlag == false" :src="dataForm.video" class="avatar" controls="controls">您的浏览器不支持视频播放</video>
          <i v-else-if="dataForm.video =='' && videoFlag == false" class="el-icon-plus avatar-uploader-icon"></i>
  ...
  export default {
    ...
    methods: {
      ...
      orgListTreeSetCurrentNode (type) {
        if (type === 'mod') {
          this.$refs.clounmListTree.setCurrentKey(this.dataForm.columnId)
          this.dataForm.cloumnName = (this.$refs.clounmListTree.getCurrentNode() || {})['columnName']
        }
      },
      // 表单提交
      dataFormSubmit () {
