<template>
  <div style="margin-left:40px;width:970px">
    ...
    <div style="margin-top: 50px;">
      <div class="block">
        <span class="demonstration">收藏范围选择： </span>
        <el-date-picker
            v-model="browse_time"
            type="datetimerange"
...
export default {
  data() {
    return {
      activeName: 'first',
      userId:'',collection,collection_anti,
      //页
      pickerOptions: {
        shortcuts: [{
          text: '最近一周',
  ...
  methods: {
    ...
    //分页
        async handleCurrentChange(pageIn) {
                ...
                // this.initData(page);
                // var res = await GoodsHttp.myCollect(this.browse_time,25,`${pageIn}`-1,this.pageSize)
                this.$http.post(`api/myCollect/wnUserCollect?page=${`${pageIn}`-1}&size=${this.pageSize}`,{
        ...
        async handleSizeChange(val) {
                ...
                this.$http.post(`api/myCollect/wnUserCollect?page=0$size=${`${val}`}`,{
                  browse_time: this.browse_time,
                  userid: this.userId
                }).then((res)=>{
                  // console.log(res);
                  // this.content = res.data.content;
                  var {list,pageSize} = res.data.biz;
    ...
    async  toQuery(){
      this.$http.post(`api/myCollect/wnUserCollect?page=${this.currentPage-1}&size=${this.pageSize}`,{
        browse_time: this.browse_time,
        userid: this.userId
      }).then((res)=>{
        // console.log(res);
      ...
      // var res = await GoodsHttp.myCollect(this.browse_time,25,0,10);
			// // this.tableData=res.data.biz.content;
      //       console.log(res.data.content);
      //       var {content,totalElements,size} = res.data.content;
