            ...
            <div class="loginContent">
                ...
                <div class="loginChoice">
                        <el-tabs v-model="activeName" @tab-click="handleClick">
                            <el-tab-pane label="手机动态绑定" name="first">
                                ...
                                <el-form class="el-form login-form el-form--label-left" :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2">
                                    <el-form-item class="el-form-item is-required el-form-item--small" label="" prop="username">
                                        <div class="el-form-item__content" style="margin-left: 0px;">
                                            ...
                                            <el-input class="CountryPhone" v-model="ruleForm2.username" autocomplete="off" placeholder="请输入手机号码" prefix-icon="el-icon-user" />
                                        </div>
                                    </el-form-item>
...
export default {
    ...
    async mounted() {
        this.$http.post('/api/getQRCodeUrl').then(res=>{
            this.wechatScan = res.data.biz
        })
        var rescode = await UserHttp.code();
            this.code = rescode.data.biz
        if(this.getUrlParam('openId')){
          this.openId = this.getUrlParam('openId')
        }
        if(this.getUrlParam('name')){
          this.wxName = this.getUrlParam('name')
        }
        if(this.getUrlParam('zfbId')){
          this.zfbId = this.getUrlParam('zfbId')
          debugger
        }
        console.log(this.zfbId)
