            ...
            <div class="merchantRegistration">
                ...
                <el-tab-pane label="" name="0">
                    <div class="stepsContent">
                        <div data-v-a72aa574="" class="el-card is-always-shadow" style="">
                                    ...
                                    <el-col :span="24">
                                        ...
                                        >
                                                <el-upload class="avatar-uploader" 
                                                    drag
                                                    action="http://api.woniuskill.com/api/uploadVideo" 
                ...
                <div class="stepsContent1">
                    <div style="margin-top: 20px; background-color: #eeeeee">
                        ...
                        <div class="tableInfro">
                            ...
                            <div class="tableCon">
                                <div class="leftTitle">
                                    <div v-for="(item,idx) in pricesCustoms1" :key="idx" class="item">
                                        <template v-if="idx < 5 || item.title == '价格'">
                                            <p style="margin-left:40px;display:inline-block">{{item.title}}</p>    
                                        </template>
                                        <template v-else>
                                            <div style="margin-left:40px;display:inline-block">
                                                <el-input v-model="item.title" placeholder="请填写配置参数名" @input="inputTitMore(idx, item.title)"></el-input>
                                            </div>
                                ...
                                <div class="valueBox">
                                    <div class="item"  v-for="(val,idx) in pricesCustoms1" :key="idx" >
                                        <template v-if="val.title == '交付时间'">
                                            <el-select v-model="val.textValue" placeholder="请选择交付时间" style="float:left;width:100%">
                                                <el-option
                                        ...
                                        <template v-else-if="val.title == '可修改次数'">
                                            <el-input v-model="val.textValue" placeholder="请输入可修改次数" type="number"></el-input>
                                        </template>
                                        <template v-else-if="val.title == '商品数量'">
                                            <el-input v-model="val.textValue" placeholder="请输入商品数量" type="number"></el-input>
                                        </template>
                                        <template v-else-if="val.title == '价格'">
                                            <el-input v-model="val.textValue" placeholder="请输入价格" onkeyup="this.value=this.value.match(/\d+\.?\d{0,2}/);this.dispatchEvent(new Event('input'))"></el-input>
                                        </template>
                                        <template v-else>
                                            <template v-if="val.isText > 0">
                                                <el-input v-model="val.textValue" placeholder="请输入服务名"  v-if="val.title == '服务名'"></el-input>
                                                <el-input v-model="val.textValue" placeholder="请输入服务描述" v-else-if="val.title == '服务描述'"></el-input>
                                                <el-input v-model="val.textValue" placeholder="请输入配置参数" v-else></el-input>
                                            </template>
                                            <el-checkbox v-model="val.isHave" true-label="true"  false-label="false" v-else style="margin-left:150px" ></el-checkbox>
                                             <div class="editBtn" v-if="val.customSort > 5 && val.title !== '价格' && settingValue == 1">
                                                <el-button type="text" @click="addCustomeLine(val,idx)">增加</el-button>
                                                <el-button type="text" @click="delCustomeLine(idx,2)" v-if="idx > 5">删除</el-button>
                                            </div>
                                        </template>
                                    </div>
                                </div>
                                <div class="valueBox">
                                    <div class="item"  v-for="(val,idx) in pricesCustoms2" :key="idx" >
                                        <template v-if="val.title == '交付时间'">
                                            <el-select v-model="val.textValue" placeholder="请选择交付时间" style="float:left;width:100%">
                                                <el-option
                                                    v-for="item in deliverTime"
                                        ...
                                        <template v-else>
                                            ...
                                            <el-checkbox v-model="val.isHave" true-label="true"  false-label="false" v-else style="margin-left:150px" ></el-checkbox>
                                            <div class="editBtn" v-if="val.customSort > 5 && val.title !== '价格' && settingValue == 2">
                                                <el-button type="text" @click="addCustomeLine(val,idx)">增加</el-button>
                                                <el-button type="text" @click="delCustomeLine(idx,2)" v-if="idx > 5">删除</el-button>
                                            </div>
                                        </template>
                                ...
                                <div class="valueBox">
                                    <div class="item"  v-for="(val,idx) in pricesCustoms3" :key="idx" >
                                        <template v-if="val.title == '交付时间'">
                                            <el-select v-model="val.textValue" placeholder="请选择交付时间" style="float:left;width:100%">
                                                <el-option
                                                    v-for="item in deliverTime"
                                        ...
                                        <template v-else-if="val.title == '可修改次数'">
                                            <el-input v-model="val.textValue" placeholder="请输入可修改次数" ></el-input>
                                        </template>
                                        <template v-else-if="val.title == '商品数量'">
                                            <el-input v-model="val.textValue" placeholder="请输入商品数量" type="number"></el-input>
                                        </template>
                                        ...
                                        <template v-else>
                                            ...
                                            <el-checkbox v-model="val.isHave" true-label="true"  false-label="false" v-else style="margin-left:150px" ></el-checkbox>
                                            <div class="editBtn" v-if="val.customSort > 5 && val.title !== '价格'">
                                                <el-button type="text" @click="addCustomeLine(val,idx)">增加</el-button>
                                                <el-button type="text" @click="delCustomeLine(idx,2)" v-if="idx > 5">删除</el-button>
                                            </div>
                                        </template>
...
export default {
    ...
    data() {
        ...
        return {
            domain: 'http://api.woniuskill.com',
            NavTitle: [],
    ...
    methods: {
        init() {
            let titleArr = ['服务名','服务描述','交付时间','可修改次数','商品数量','价格']
                ...
                let newArr3 = JSON.parse(JSON.stringify(newArr))
                newArr3.forEach(item => {
                    item.level = 3
                })
                this.pricesCustoms1 = newArr
                this.pricesCustoms2 = newArr2
                this.pricesCustoms3 = newArr3
        },
        getDetail() {
                ...
                if (res.data.biz.files.length > 0) {
                    ...
                    res.data.biz.files.forEach(item => {
                        if (item.fileType == 3) {
                            ...
                            let obj = {
                                name: '',
                                url: 'http://api.woniuskill.com' + item.fileUrl
                            }
                            imgArr.push(obj)
                        }
                    })
                    this.addform.imgs2 = imgArr
                    ...
                    if (res.data.biz.askfors) {
                        this.askfors = res.data.biz.askfors
                    }
                    //配置参数
                    let customLen = res.data.biz.pricesCustoms.length
                    let newArr = JSON.parse(JSON.stringify(res.data.biz.pricesCustoms))
                    if (this.settingValue == 3) {
                        this.pricesCustoms1 = newArr.splice(0, customLen / 3);
                        this.pricesCustoms2 = newArr.splice(0, customLen / 3);
                        this.pricesCustoms3 = newArr.splice(0, customLen / 3);
                    } else if (this.settingValue == 2){
                        this.pricesCustoms1 = newArr.splice(0, customLen / 2);
                        this.pricesCustoms2 = newArr.splice(0, customLen / 2);
                    } else {
                        this.pricesCustoms1 = newArr
                    }
                    // 常见问题
        ...
        handleContImgRemove: function (file, fileList) {
            // this.addform.imgs = fileList;
            // this.addform.imgs.remove(file.url);
            this.addform.imgs.splice(0,  this.addform.imgs.length, ...fileList);
        },
        handleAvatarSuccess(res) {
            this.addform.imgs.push({fileType:1, url: res[0].fileUrl});
        },
        ...
        uploadImage(params) {
            ...
            this.$http.post('http://api.woniuskill.com/api/uploadImage',form).then(res => {
                this.addform.imgs.push({fileType:1, url: res.data[0].fileUrl});
            });
        },
        //内容图数量限制3张
        exceedTips: function () {
        ...
        submitAdd() {
            let dt = {
                ...
                askfors: this.askfors.join(','),
                categoryId: this.addform.categoryId,
                createUser: 19,
                expressDeliveries: this.expressDeliveries,
                faq: this.faq,
                ...
                orderQuantity: this.addform.serviceStock
            }
            let newArr = [];
            if (this.settingValue == 1) {
                dt.pricesCustoms = this.pricesCustoms1
            } else if (this.settingValue == 2) {
                dt.pricesCustoms = this.pricesCustoms1.concat(this.pricesCustoms2)
            } else {
                dt.pricesCustoms = this.pricesCustoms1.concat(this.pricesCustoms2,this.pricesCustoms3)
            }
        ...
        addAsk() {
            ...
            this.askfors.push(this.askfors_text);
            this.dialogVisible = false;
            this.askfors_text = ''
        },
        addCustomeLine(val,index) {
            if (val.isText && !val.textValue || !val.title) {
            ...
            this.pricesCustoms1.splice(index + 1, 0, obj)
            this.pricesCustoms2.splice(index + 1, 0, obj2)
            this.pricesCustoms3.splice(index + 1, 0, obj3)
        },
        delCustomeLine(index, type) {
            this.pricesCustoms1.splice(index, 1)
            this.pricesCustoms2.splice(index, 1)
            this.pricesCustoms3.splice(index, 1)
        },
        delzjfw(idx) {
        ...
        changeText(e,idx) {
            let arr = this.pricesCustoms2[idx];
            arr.isText = e
            let arr2 = this.pricesCustoms3[idx];
            arr2.isText = e
        },
        inputTitMore(idx, text) {
            this.pricesCustoms2.forEach((item,index) => {
                if (index == idx) {
                    item.title = text
                }
            })
            this.pricesCustoms3.forEach((item,index) => {
                if (index == idx) {
                    item.title = text
                }
            })
        ...
        nextStepSecong() {
            if (this.settingValue == 1) {
                let arr = this.pricesCustoms1;
                for (const i in arr) {
                    if (arr[i].isText && arr[i].isTitle && !arr[i].textValue) {
                ...
                let arr2 = this.pricesCustoms2;
                for (const i in arr2) {
                    if (arr2[i].isText && arr2[i].isTitle && !arr2[i].textValue) {
                        this.$message.error('请填写' +arr2[i].title)
                        return false
                    }
                }
                if (this.showExp) {
                ...
                let arr3 = this.pricesCustoms3
                for (const i in arr3) {
                    if (arr3[i].isText && arr3[i].isTitle && !arr3[i].textValue) {
                        this.$message.error('请填写' +arr[i].title)
                        return false
                    }
                }
        ...
        handleVideoSuccess(res, file) {                               //获取上传图片地址
            ...
            if(res){
                ...
                this.addform.videoUrl.push({ fileType: 3, url:res[0].fileUrl });
            }else{
