<template>
    <div class="show-imgs-container" @mouseover="isIn = true" @mouseout="isIn = false">
        <el-carousel
            ...
            @change="getIndex"
        >
            <el-carousel-item v-for="(bigImgitem, index) in bigImg" :key="index">
                <!--视频播放器 -->
                <video :ref="'videoPlayer' + index" :controls="isIn">
                    <source :src="item.src" type="video/mp4" />
                    您的浏览器不支持Video标签。
                </video>
                <div v-if="bigImgitem.fileTyle == 3" style="width: 720px; height: 425px">
                    <video :ref="'videoPlayer' + index" :controls="isIn" style="width: 720px; height: 425px">
                        <source :src="item.fileUrl" type="video/mp4">
                        您的浏览器不支持Video标签。
                    </video>
                </div>
        <div v-if="bigImgitem.fileType == 1" style="width: 720px; height: 425px">
            <img style="width: 720px; height: 425px"
                  class="img"
                  :src="getImgUrl(bigImgitem.fileUrl)"
              />
        </div>
